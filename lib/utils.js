  ...
  for (var i = 0, l = rules.length; i < l; i++) {
    ...
    */
    ...
    if (rules[i].selectorText &&
        (rules[i].selectorText.indexOf('hover') >= 0 ||
        rules[i].selectorText.indexOf('active') >= 0 ||
        rules[i].selectorText.indexOf('focus') >= 0 ||
        rules[i].selectorText.indexOf('visited') >= 0 ||
        rules[i].selectorText.indexOf('link') >= 0)) {
        var stateRule = [ '' ];
        stateRule.push(rules[i].selectorText + '{');
        stateRule = stateRule.concat(getStringifiedStyles(rules[i]));
        stateRule.push('}');
        if (stateRule.length) {
            preserved.push(stateRule.length ? stateRule.join(os.EOL) + os.EOL : '');
        }
    }
